...
type 'a message =
  ...
  ; mutable client_stream : Stream.t
  ; mutable server_stream : Stream.t
...
let meth request = request.specific.meth
let target request = request.specific.target
let set_method request meth = request.specific.meth <- meth
let set_target request target = request.specific.target <- target

(* Responses *)

let response ?status ?(headers = []) client_stream server_stream =
  let status = Status.default status in
  { kind = Response
  ; specific = { status }
  ; headers
  ; client_stream
...
let header message name =
  try Some (header_basic name message) with
  | Not_found -> None
;;
...
let headers message name =
  ...
  |> List.fold_left
       (fun matched (name', value) ->
         if String.lowercase_ascii name' = name then value :: matched else matched)
       []
  |> List.rev
;;
...
let add_header message name value = message.headers <- message.headers @ [ name, value ]
...
let lowercase_headers message =
  let headers =
    message.headers |> List.map (fun (name, value) -> String.lowercase_ascii name, value)
  in
...
let set_content_length_headers message =
  if has_header message "Content-Length"
  then ()
  else if has_header message "Transfer-Encoding"
  then ()
  else (
    match message.body with
    | None -> add_header message "Transfer-Encoding" "chunked"
    | Some body_promise ->
      failwith "set_content_length_headers: body_promise is not None"
      (* (match body_promise with *)
      (*  | None -> add_header message "Transfer-Encoding" "chunked" *)
      (*  | Some body -> *)
...
let flush stream =
  ...
  (* promise *)
  failwith "message.ml: flush"
;;

let close stream = Stream.close stream 1000
let client_stream message = message.client_stream
let server_stream message = message.server_stream
let set_client_stream message client_stream = message.client_stream <- client_stream
let set_server_stream message server_stream = message.server_stream <- server_stream

(* TODO: I think this would be better with a few modules *)
...
(*         let string = *)
(*           Bigstringaf.sub buffer ~off:offset ~len:length |> Bigstringaf.to_string *)
(*         in *)
(*         let text_or_binary = if binary then `Binary else `Text in *)
(*         let end_of_message = if fin then `End_of_message else `Continues in *)
(*         Lwt.wakeup_later resolver (Some (string, text_or_binary, end_of_message))) *)
(*       ~flush:loop *)
...
let set_field message key value = message.fields <- Fields.add key value message.fields
